...
const appId = 'kas-harian-blogger'; // Gunakan ID yang konsisten
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function updateVoucherNumberDisplay() {
        ...
        const counterDocRef = doc(db, `artifacts/<span class="math-inline">\{appId\}/users/</span>{userId}/groupData/<span class="math-inline">\{selectedGroup\}/voucherCounters/</span>{yearMonth}`);
        ...
        try {
            ...
            voucherNumberDisplay.textContent = `<span class="math-inline">\{voucherType\}\-</span>{targetVoucherMonthYearForDisplay}-${formattedSeq}`;
            showLoading(false);
        }
    }
    ...
    if (addTransactionButton) {
        addTransactionButton.addEventListener('click', async () => {
            ...
            try {
                // Run a transaction to atomically get and update the voucher number
                ...
                // If transaction is successful, construct the full voucher number
                const formattedSeq = String(finalSequenceNumber).padStart(3, '0');
                const voucherMonthYearForNumber = getMonthYear(tgl); // e.g., "0625"
                finalVoucherNumber = `<span class="math-inline">\{tipe\}\-</span>{voucherMonthYearForNumber}-${formattedSeq}`;

                // Now, save the actual transaction document
                ...
                // For updates, we just overwrite data, voucher number does not change
                if (editingTransactionId) {
                    // Re-fetch original transaction to preserve its voucher number
                    const txDocRef = doc(db, `artifacts/<span class="math-inline">\{appId\}/users/</span>{userId}/groupData/<span class="math-inline">\{selectedGroup\}/dailyCashReports/</span>{currentReportDate}/transactions/${editingTransactionId}`);
                    const originalTxSnap = await getDoc(txDocRef);
                    if (originalTxSnap.exists()) {
                        const originalTx = originalTxSnap.data();
                        transactionData.noVoucher = originalTx.noVoucher;
                        transactionData.voucherNumberSequence = originalTx.voucherNumberSequence;
                        transactionData.voucherMonthYear = originalTx.voucherMonthYear;
                        transactionData.createdAt = originalTx.createdAt; // Preserve original creation date
                ...
                console.error("Gagal menyimpan transaksi (voucher counter error): ", error);
    ...
    async function renderReport() { 
        ...
        // --- START PERUBAHAN ---
        // Variabel baru khusus untuk aturan YULI
            ...
            transactions.forEach(tx => {
                ...
                if (tx.voucherType === 'KM') {
                    ...
                    if (tx.category === 'OPERASIONAL') {
                        ...
                        currentPeriodKK_Operasional += tx.totalKas; // Aturan YULI
                    }
                    if (tx.category === 'ONDERDIL') {
                        algebraicSumOnderdil += tx.totalKas;
                        currentPeriodKK_Onderdil += tx.totalKas; // Aturan YULI
                    }
                    if (tx.category === 'UMUM') {
                        algebraicSumUmum += tx.totalKas;
                        currentPeriodKK_Umum += tx.totalKas; // Aturan YULI
                    }
                    if (tx.category === 'KANTOR') {
                        algebraicSumKantor += tx.totalKas;
                        currentPeriodKK_Kantor += tx.totalKas; // Aturan YULI
                    }
                }
            });
        }
            ...
            try {
                const canvas = await html2canvas(reportElement, { 
                    ...
                    onclone: (documentClone) => { 
                        const tableCells = documentClone.querySelectorAll('.report-container table th, .report-container table td');
                        tableCells.forEach(cell => {
                            // Optional: Explicitly set styles again for html2canvas if needed
    ...
    function generateReportHTML() {
        ...
        if(table){
            ...
            // Remove the "AKSI" column from the clone
        ...
        copyTableButton.addEventListener('click', () => {
            ...
            try {
                ...
                if (newWindow) {
                    ...
                    // Re-using the same style block from generateReportHTML
                    ...
                    // We need just the table part, not the whole HTML doc inside the body
                    const reportContainer = document.querySelector('.report-container');
                    if(reportContainer) {
                        newWindow.document.write(reportContainer.innerHTML);
                        newWindow.document.querySelectorAll('.no-print').forEach(el => el.remove());
                    }
